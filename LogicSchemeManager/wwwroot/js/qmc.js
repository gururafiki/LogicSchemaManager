...
function QuineMcCluskeyDataCtrl() {
    ...
    this.applyData = function (truthTableOutputs, inputsNames, useInputNames, useColors) {
        this.inputsNames = inputsNames;
        this.useInputNames = useInputNames;
        this.useColors = useColors;
        for (var i = 0; i < truthTableOutputs.length; i++) {
            this.funcdata[i] = truthTableOutputs[i];
        }
        this.compute();
    };
                    ...
                    if (!containsOne) {
                        ...
                        for (var thisVal in primTerm.implicant.imp) {
                            ...
                            for (var v = 0; v < this.noOfVars; v++) {
                                var namePair = {};
                                var index = v;
                                if (this.inputsNames && this.useInputNames) {
                                    var pair = this.inputsNames[v].split('_');
                                    namePair.element = pair[0].toLowerCase();
                                    namePair.index = pair[1].toLowerCase();
                                    index = this.inputsNames[v].toLowerCase();
                                }
                                else {
                                    this.useInputNames = false;
                                }

                                if ((needed & one) === one) {
                                    if ((thisVal & one) === one) {
                                        minTerm = this.useInputNames ? ("<i>x</i><sub><small>" + index + "</small></sub> " + minTerm) : ("<i>x</i><sub><small>" + v + "</small></sub>" + minTerm);
                                    } else {
                                        minTerm = this.useInputNames ? ("<i>x&#772;</i><sub><small>" + index + "</small></sub> " + minTerm) : ("<i>x&#772;</i><sub><small>" + v + "</small></sub>" + minTerm);
                                    }
                                }
                            ...
                            var colorStr = this.useColors ? "rgb(" + primTerm.color[0] + "," + primTerm.color[1] + "," + primTerm.color[2] + ")" : "black";
...
function QuineMcCluskey(parentDivId, columns, language) {
    ...
    this.update = function () {
        ...
        // clean up
        var myInnerDiv = document.getElementById(divId + "_innerDiv");
        if (myInnerDiv) myInnerDiv.parentElement.removeChild(myInnerDiv);
        ...
        */
        ...
        termDiv.innerHTML = "<p ><span class='qmcMathFont'><i>y</i>&nbsp;=&nbsp;" + this.data.coloredMinimalTerm; +"</span></p>";
